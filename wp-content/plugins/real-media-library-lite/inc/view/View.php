...
class View
{
    ...
    {
        $options = \wp_parse_args($options, ['selected' => \_wp_rml_root(), 'disabled' => [], 'nullable' => \false, 'editable' => \true, 'name' => \false, 'title' => null]);
     ...
     * @param boolean $disabled
     */
    private function optionsHTML($selected = -1, $tree = null, $slashed = '', $spaces = '&nbsp;&nbsp;', $useAll = \true, $disabled = null)
    {
        $return = '';
        ...
        if (!\is_null($tree) && \count($tree) > 0) {
            foreach ($tree as $parent) {
                ...
                $return .= '<option value="' . $parent->getId() . '" ' . $this->optionsSelected($selected, $parent->getId()) . ' data-path="/' . \esc_attr($parent->getAbsolutePath()) . '"' . ' data-name="' . \esc_attr($parent->getName()) . '"' . ' data-type="' . $parent->getType() . '"' . (\in_array($parent->getType(), $disabled, \true) ? ' disabled="disabled" ' : '') . '>' . $spaces . '&nbsp;' . $parent->getName(\true) . '</option>';
    ...
    {
        if (\is_array($selected) && \in_array(
            $value,
            $selected,
            // Explicitly allow string in array to be e.g. compatible with ACF field plugin
            \false
        ) || $selected === $value) {
