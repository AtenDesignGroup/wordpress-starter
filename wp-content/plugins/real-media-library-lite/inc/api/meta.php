...
if (!\function_exists('add_rml_user_settings_box')) {
    ...
    {
        if (!Meta::getInstance()->add($name, $obj)) {
            return \false;
        }
        \add_filter('RML/User/Settings/Content' . (empty($contentGroup) ? '' : '/' . $contentGroup), [$obj, 'content'], $priority, 2);
...
if (!\function_exists('add_rml_meta_box')) {
    ...
    function add_rml_meta_box($name, $obj, $hasScripts = \false, $priority = 10, $contentGroup = '')
    {
        ...
        \add_filter('RML/Folder/Meta/Content' . (empty($contentGroup) ? '' : '/' . $contentGroup), [$obj, 'content'], $priority, 2);
