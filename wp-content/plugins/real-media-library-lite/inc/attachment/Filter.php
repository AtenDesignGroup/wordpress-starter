...
class Filter
{
    ...
    public function getAttachmentFolder($attachmentId, $default = null)
    {
        ...
        $attachmentIds = $isArray ? $attachmentId : [$attachmentId];
        if (\count($attachmentIds) > 0) {
            global $wpdb;
            $attachments_in = \implode(',', \array_map('intval', $attachmentIds));
    ...
    public function posts_clauses($clauses, $query)
    {
         ...
         * @param {int|int[]} $folderId The folder ID(s) to query (0 = 'All files')
            ...
            if ($folderId > 0 || $folderId === $root) {
                if ($folderId > 0) {
                    ...
                    if ($includeChilds && \wp_rml_all_children_sql_supported(\false, 'function')) {
                        ...
                        // phpcs:enable WordPress.DB.PreparedSQL
                    } elseif (\is_array($folderId)) {
                        if (\count($folderId) > 0) {
                            $clauses['join'] .= \sprintf(' AND rmlposts.fid IN (%s) ', \join(',', \array_map(function ($fid) use($wpdb) {
                                return $wpdb->prepare('%s', $fid);
                            }, $folderId)));
                        } else {
                            // An empty array may not match anything
                            $clauses['join'] .= ' AND rmlposts.fid = -99';
                        }
                    } else {
    ...
    public function pre_get_posts($query)
    {
        ...
        $folder = $this->getFolder($query, $assets->isScreenBase('upload') || $assets->isScreenBase(Assets::MLA_SCREEN_BASE));
    ...
    public function getFolder($query, $fromRequest = \false)
    {
        ...
        if ($id === DefaultFolder::ID_LAST_QUERIED) {
    ...
    {
        // Reset folder count
        \MatthiasWeb\RealMediaLibrary\attachment\CountCache::getInstance()->resetCountCacheOnWpDie(\wp_attachment_folder($postID));
    ...
    {
        $isGridMode = \is_admin() && \defined('DOING_AJAX') && \constant('DOING_AJAX') && isset($_REQUEST['action']) && $_REQUEST['action'] === 'query-attachments';
        $assets = $this->getCore()->getAssets();
        $isListMode = $assets->isScreenBase('upload');
        $isMLAPage = $assets->isScreenBase(Assets::MLA_SCREEN_BASE);
