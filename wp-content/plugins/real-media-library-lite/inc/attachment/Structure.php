...
class Structure implements IStructure
{
    ...
    private function fetch()
    {
         ...
         * @return {array}
         */
        $fields = \join(', ', \apply_filters('RML/Tree/SQLStatement/SELECT', [
            // The whole row of the folder
            'tn.*',
            // Count images for this folder
            'IFNULL(tn.cnt, (' . \MatthiasWeb\RealMediaLibrary\attachment\CountCache::getInstance()->getSingleCountSql() . ')) AS cnt_result',
         ...
         * @return {array} $fields
         */
        $joins = \join(' ', \apply_filters('RML/Tree/SQLStatement/JOIN', [
         ...
         * @return {string}
         */
        $sqlStatement = \apply_filters('RML/Tree/SQLStatement', ["SELECT {$fields} FROM {$table_name} AS tn {$joins} {$where} ORDER BY parent, ord"], $this);
    ...
    private function parse()
    {
        ...
        if (!empty($this->rows)) {
            ...
            foreach ($this->rows as $key => $value) {
                ...
                // Craetable instance
                $creatable = CRUD::getInstance()->getCreatables($value->type);
                if (isset($creatable)) {
    ...
    public static function getInstance()
    {
        ...
        return !isset(self::$blogs[$bid]) ? self::$blogs[$bid] = new \MatthiasWeb\RealMediaLibrary\attachment\Structure() : self::$blogs[$bid];
    }
}
