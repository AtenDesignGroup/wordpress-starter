...
class BaseViewer {
	...
	set currentPageLabel(val) {
		...
		// The intent can be to just reset a scroll position and/or scale.
		if (
			!this._setCurrentPageNumber(page, /* resetCurrentPageView = */ true)
		) {
		...
		Promise.all([firstPagePromise, permissionsPromise])
			.then(([firstPdfPage, permissions]) => {
				...
				const textLayerFactory =
					textLayerMode !== TextLayerMode.DISABLE && !isPureXfa
						? this
						: null;
	...
	getPagesOverview() {
		return this._pages.map((pageView) => {
			...
			if (
				...
				isPortraitOrientation(viewport)
			) {
				return {
	...
	nextPage() {
		...
		const advance =
			this._getPageAdvance(currentPageNumber, /* previous = */ false) ||
			1;
