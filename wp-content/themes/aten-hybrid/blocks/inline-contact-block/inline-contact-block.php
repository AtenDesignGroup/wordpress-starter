...
if( isset( $block['data']['preview_image'] )  ) :    /* rendering in inserter preview  */
	...
	if( have_rows('inline_contact_info', $post_id) ):
		while( have_rows('inline_contact_info', $post_id) ): the_row();
						...
						<h2 class="title">Contact Information</h2>
								...
								<?php if ($address) : ?>
									<p>
										...
										<?php if ($maps_link) { ?> <a href="<?php echo $maps_link; ?>" target="_blank" title="Get directions"> <?php } ?>
											<?php echo ($address . ', ' . $city . ', ' . $state . ' ' . $zip); ?>
										<?php if ($maps_link) { ?> </a> <?php } ?>
									</p>
								<?php endif; // Address ?>
								...
								<?php if (is_array($hours_of_operation)):
									...
									if( have_rows('inline_contact_info', $post_id) ): while( have_rows('inline_contact_info', $post_id) ): the_row();
										?>
