...
if( isset( $block['data']['preview_image'] )  ) :    /* rendering in inserter preview  */
	...
	$title = !empty($custom_title) ? $custom_title : get_the_title($referenced_post_link);
	$image_id = !empty($custom_image) ? $custom_image : ($has_featured_image ? get_post_thumbnail_id($referenced_post_link) : '');

	?>

	<div <?php echo $anchor; ?> class="l-gutter callout-link-component <?php if (!$image_id) { echo "callout-without-image "; }; echo esc_attr($class_name); ?>">
		<div class="callout-link-text">
			<?php if ($referenced_post_link) : ?>
				...
				<hr class="gold" />
				<?php if( $teaser ) { ?><div class="callout-teaser"><?php echo ( $teaser ); ?></div><?php } ?>
			<?php endif; ?>
		</div>
