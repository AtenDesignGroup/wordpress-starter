...
jQuery( document ).ready(function($) {
    ...
    $('#mega-menu-wrap-main-nav .mega-toggle-label .mega-toggle-label-open').remove();
    $('#mega-menu-wrap-main-nav .mega-toggle-label .mega-toggle-label-closed').addClass('mega-toggle-label-open');

    // Adding icon before and after Translate link
    $('<span class="menu-icon">language</span>').appendTo('.gtranslate_wrapper');
    $('<span class="menu-icon right">expand_more</span>').appendTo('.gtranslate_wrapper');

    // Preventing icon spans from being translated so that icons remain the same regardless of language selected
    $('.menu-icon, .mobile-menu-icon, .wp-block-search__button').addClass('notranslate');
    ...
    $(".mega-sub-menu").parent().on("keyup.megamenu", function(e) {
    ...
    function activateButtonTriggers() {
        ...
        if(screen_size < 1024) { 
            ...
            $('button.mega-menu-button').unbind('keypress keyup').on('keypress', function(e){
            ...
            // Triggering mega menu using top level buttons on enter key
            $('button.mega-menu-button').unbind('keypress keyup').on('keydown', function(e){
                // Listening for the press of the enter key
                if(e.which == 13){
                    e.preventDefault();
                    e.stopImmediatePropagation();
    ...
    function arrangeMenus() {
        ...
        if(screen_size < 768) {
            ...
            if(!$('header').hasClass('mobile-layout')) {
                ...
                if($('header').hasClass('tablet-layout')) {
                    ...
                    $('#mega-menu-primary').children('li').addClass('utility-items').appendTo('#mega-menu-main-nav');
                    $('.gtranslate_wrapper').wrap('<li id="mobile-translate-widget"></li>');
                    $('#mobile-translate-widget').appendTo('#mega-menu-main-nav');
                }
                // Move the search into the main nav
                $('header form.wp-block-search').wrap('<li id="mobile-search-widget"></li>');
        ...
        // Tablet layout
            ...
            if(!$('header').hasClass('tablet-layout')) {
                ...
                if($('header').hasClass('mobile-layout')) {
                    ...
                    $('header form.wp-block-search').detach().insertAfter('#mega-menu-wrap-primary');
        ...
        // Desktop layout
            ...
            if($('header').hasClass('tablet-layout') || $('header').hasClass('mobile-layout')) {
                ...
                $('#mega-menu-main-nav').children('.utility-items').appendTo('#mega-menu-primary').removeClass('utility-items');
                // Move the search form back into the utility bar
                $('header form.wp-block-search').detach().insertAfter('#mega-menu-wrap-primary');
                // Move the translation widget back into the utility bar
                $('.gtranslate_wrapper').detach().insertBefore('header form.wp-block-search');
    ...
    function standardizeEscapeFunctionality() {
        ...
        if(screen_size < 1024) { 
            ...
            $('.mega-menu-item-has-children .mega-sub-menu .mega-menu-link').on('keydown', function(e){
                // Listening for the press of the ESC key
                if(e.key === "Escape") {
                    // Preventing default collapse behavior
                    e.stopImmediatePropagation();
                    // Set focus to submenu parent item
                    let parent_btn = $(this).closest('.mega-menu-item-has-children').find('.mega-menu-button');
                    $(parent_btn).focus();
            ...
            $('.mega-menu-item-has-children > .mega-menu-button').on('keydown', function(e){
                // Listening for the press of the ESC key
                if(e.key === "Escape") {
    ...
    function triggerMegaMenu(button) {
        ...
        if($(button).attr('aria-expanded') === 'false' && !$(item).hasClass("menu-toggle-on")) {
