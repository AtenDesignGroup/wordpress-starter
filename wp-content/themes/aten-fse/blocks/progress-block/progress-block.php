...
if ( isset( $block['data']['preview_image'] ) ) :    /* rendering in inserter preview. */
	...
	// Loop through Column.
	if ( have_rows( 'column' ) ) : ?>
		<div <?php echo esc_attr( $anchor ); ?>class="<?php echo esc_attr( $class_name ); ?> l-full">
			<div class="progress-block-component">
				...
				<div class="progress-block-columns">
					<ul class="progress-block-list-wrapper col-count-<?php echo esc_attr( $column_count ); ?>">
							<?php
							...
							<li class="progress-block-column">
								...
								<ul class="progress-links">
									...
									while ( have_rows( 'link' ) ) :
										the_row();
										$progress_link = get_sub_field( 'link_url' );
										$link_title    = get_sub_field( 'link_title' );
										$link_icon     = get_sub_field( 'link_icon' );
										$link_target   = isset( $progress_link['target'] ) ? $progress_link['target'] : '_self';
										?>
										<li class="progress-link">
											<!-- Link loop content -->
											<a href="<?php echo esc_url( $progress_link['url'] ); ?>" title="<?php echo esc_attr( $progress_link['title'] ); ?>" target="<?php echo esc_attr( $link_target ); ?>">
		...
		<div class="progress-block-border" aria-hidden="true">
			<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/progress-border.svg" alt="" />
		</div>
